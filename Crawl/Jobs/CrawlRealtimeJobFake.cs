...
namespace Crawl.Jobs
{
    ...
    public class CrawlRealtimeJobFake : IJob
    {
            ...
            try
            {
                ...
                foreach (var item in lstLink)
                {
                    try
                    {
                        ...
                        //Insert Sqlite
                        if (!string.IsNullOrWhiteSpace(model.MaSoThue) && !SqliteMng.CheckExist(model.MaSoThue) && !string.IsNullOrWhiteSpace(model.TinhThanh))
                        {
                            if(!_lClause.Any())
                            {
        ...
        {
            var val = StaticVal._config.TinhThanh;
